<template>
  <div class="bag" style="margin: 0 auto; padding-top: 80px">
    ...
    <table v-else>
      ...
      <tr v-for="(product, item) in products" :key="item">
        <td>{{ product.name }}</td>
        <td>{{ products[item].price * products[item].pivot.quantity }} $</td>
        <td>
          <span
            class="button_item"
            @click="decrementItem(product.pivot.id, item)"
            >-</span
          >
          {{ products[item].pivot.quantity }}
          <span
            class="button_item"
            @click="incrementItem(product.pivot.id, item)"
            >+</span
          >
        </td>
        <td>
...
<!--            />-->
    ...
    <div class="total">
      ...
      <router-link
        v-if="total > 0"
        :to="{ name: 'ShoppingInformation', params: { price: total } }"
  ...
  data() {
    return {
      fields: [
        ...
        "price",
        "images",
        { label: "Quantity", key: "pivot.quantity" },
        "action",
      ],
      products: [],
      selected: 1,
